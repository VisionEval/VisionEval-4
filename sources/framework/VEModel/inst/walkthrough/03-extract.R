...
print(length(datastore.list))    # number of fields in the results (should be 843)
...
# 'Units' are what in the Datastore; 'ModuleUnits' are the default
# units for export, based on whatever units were in use when the
# field was originally created in the Datastore.
...
# More useful is exporting everything to output files
...
# There will be two entries: the model directory, and the path within
# the model results that contains the outputs.
...
metadata[sample(nrow(metadata),10),]
...
print(sl) # Just the Worker and Vehicle tables selected

results$export() # uses selection "sl" implicitly due to select=TRUE
exporter <- results$export(selection=results$find(Group="Year",Table="Worker")) # exports using an explicit selection
# the "sl" selection is unchanged, and until one uses find with select=TRUE again,
# it will remain the export default.

# "exporter" is a summary of the export operation (listing location of output and tables created )
print(exporter)
...
# This will use a default name for the SQLite database ("<Scenario>_SQLite.sqlite")
# Note that using "sql" export format puts a Timestamp on each table name in case
# multiple exports end up in the same database.

exporter <- results$export("sqlite",connection=list(Database="My-SQLite-Database"))
# Using the "sqlite" export format puts a Timestamp on the database filename itself
# Compare the output to the following (which also produces a sqlite database, but
# does not incorporate a Timestamp in the file name).
exporter <- results$export("sql",connection=list(Database="My-SQLite-Database"))

# See the databases
print(mwr$dir(outputs=TRUE))

# Note that R keeps the SQLite databases open until the exporter is "garbage collected"
# which you can hasten by calling gc() or by exiting the R session
# If you create an SQLite database, be sure to "turn it off" in one of those
# ways before opening it from the outside.
...
# default will give you a list of data.frames (standard R structures)
list.of.data.frames <- exporter$data()
print(names(list.of.data.frames)) # same names as the tables
rm(list.of.data.frames)

# you can request certain other formats; e.g. data.tables are useful in the tidyverse
